...
//go:generate minimock -i github.com/s0vunia/platform_common/pkg/db.Client -o client_minimock.go -n ClientMock -p mocks
...
func (m *ClientMock) MinimockCloseInspect() {
	...
	if !m.CloseMock.invocationsDone() && afterCloseCounter > 0 {
		m.t.Errorf("Expected %d calls to ClientMock.Close but found %d calls",
...
func (m *ClientMock) MinimockDBInspect() {
	...
	if !m.DBMock.invocationsDone() && afterDBCounter > 0 {
		m.t.Errorf("Expected %d calls to ClientMock.DB but found %d calls",
			mm_atomic.LoadUint64(&m.DBMock.expectedInvocations), afterDBCounter)
	}
}
